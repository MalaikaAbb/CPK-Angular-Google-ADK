  ...
  template: `
    ...
    <div class="space-y-6">
      ...
      <ui-callout title="The agent has to agree on the state shape">
        ...
        <code>{{ '{' }} notes, priority {{ '}' }}</code>. The Google ADK agent
        in <code>backend/main.py</code> has to carry that same shape in its ADK
        session state, and expose it to the model so it can both read and update
